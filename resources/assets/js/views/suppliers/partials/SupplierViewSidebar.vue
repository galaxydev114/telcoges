  ...
  >
    ...
    >
      <router-link
        ...
        :to="`/admin/contacts/suppliers/${supplier.id}/view`"
        :key="index"
      ...
      >
        ...
        <div class="flex-1 whitespace-nowrap right">
          <div
            ...
            v-html="$utils.formatMoney(supplier.due_amount, supplier.currency)"
          />
        </div>
      </router-link>
      <p
...
export default {
  ...
  methods: {
    ...
    async loadSuppliers() {
      let response = await this.fetchSuppliers({
        limit: 'all',
      })
    ...
    scrollToSupplier() {
      const el = document.getElementById(`supplier-${this.$route.params.id}`)
    ...
    async loadSupplier() {
      ...
      this.currency = this.selectedViewSupplier.currency
    },
    async onSearch() {
      let data = {}
      if (
