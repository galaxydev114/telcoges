<template>
  <base-page class="supplier-create">
    <form v-if="!initLoad" @submit.prevent="submitSupplierData">
      ...
      <sw-card variant="supplier-card">
        ...
        <div class="grid grid-cols-5 gap-4 mb-8">
          ...
          >
            ...
            <sw-input-group
              :label="$t('suppliers.primary_contact_name')"
              class="md:col-span-3"
            >
              <sw-input
        ...
        <div class="grid grid-cols-5 gap-4 mb-8">
          ...
          <div
            class="grid col-span-5 lg:col-span-4 gap-y-6 gap-x-4 md:grid-cols-6"
          >
            <sw-input-group :label="$t('suppliers.name')" class="md:col-span-3">
              <sw-input
            ...
            <sw-input-group :label="$t('suppliers.city')" class="md:col-span-3">
              <sw-input
            ...
            <div class="md:col-span-3">
              <sw-input-group :label="$t('suppliers.phone')" class="mb-6">
                <sw-input
        ...
        >
          <div class="p-1">
            ...
            >
              ...
              <span class="text-xs">
                {{ $t('suppliers.copy_billing_address') }}
              </span>
            </sw-button>
          </div>
        </div>
...
export default {
  ...
  computed: {
    ...
    isEdit() {
      if (this.$route.name === 'contacts.suppliers.edit') {
        return true
      }
      return false
    },
    pageTitle() {
      if (this.$route.name === 'contacts.suppliers.edit') {
        return this.$t('suppliers.edit_supplier')
      }
      return this.$t('suppliers.new_supplier')
    },
    hasBillingAdd() {
  ...
  mounted() {
    ...
    this.setInitialCustomFields('Supplier')
  },
  methods: {
    ...mapActions('supplier', [
      'addSupplier',
    ...
    async loadSupplier() {
      ...
      let response = await this.fetchSupplier(params)

      this.formData = { ...this.formData, ...response.data.supplier }

      if (response.data.supplier.billing_address) {
        this.billing = response.data.supplier.billing_address

        if (response.data.supplier.billing_address.country_id) {
          this.billing_country = response.data.supplier.billing_address.country
        }
      }

      if (response.data.supplier.shipping_address) {
        this.shipping = response.data.supplier.shipping_address

        if (response.data.supplier.shipping_address.country_id) {
          this.shipping_country =
            response.data.supplier.shipping_address.country
        }
      }

      this.formData.currency_id = response.data.supplier.currency_id
      this.currency = response.data.supplier.currency

      let res = await this.fetchCustomFields({ type: 'Supplier', limit: 'all' })
      let customFields = res.data.customFields.data
      this.formData.fields = response.data.supplier.fields

      this.setEditCustomFields(response.data.supplier.fields, customFields)
    },
    async fetchInitData() {
      this.initLoad = true
    ...
    async submitSupplierData() {
      ...
      try {
        ...
        if (this.isEdit) {
          response = await this.updateSupplier(this.formData)
          if (response.data.success) {
            this.$router.push(
              `/admin/contacts/suppliers/${response.data.supplier.id}/view`
            )
            window.toastr['success'](this.$t('suppliers.updated_message'))
          }
          if (response.data.error) {
            window.toastr['error'](this.$t('validation.email_already_taken'))
          }
        } else {
          response = await this.addSupplier(this.formData)
          if (response.data.success) {
            this.$router.push(
              `/admin/contacts/suppliers/${response.data.supplier.id}/view`
            )
            window.toastr['success'](this.$t('suppliers.created_message'))
          }
        }
